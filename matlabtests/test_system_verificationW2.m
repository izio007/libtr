...
for s_idx = 1:length(STATION_COUNTS_VECTOR)
    ...
    for k = 1:TRAJECTORY_POINTS_COUNT
        ...
        alpha_measured = zeros(M, 1); beta_measured = zeros(M, 1);
        ...
        % Вызов новой нелинейной функции позиционирования
        [st_gn, lambda_gn] = gn3d_position(STATION_POSITIONS, alpha_measured, beta_measured);
        status_position_gn(k) = st_gn;
        
        if st_gn == 0
            results_pos_gn(:, k) = lambda_gn;
            % Вызов устойчивой SVD-функции расчета ковариации
% [Внутри измерительного цикла главного скрипта]
[st_c, std_X, std_Y, std_Z] = gn3d_covariance(STATION_POSITIONS, VAR_ALPHA, VAR_BETA, lambda_gn);
if st_c == 0
    fixed_results_std_X(k) = std_X;
    fixed_results_std_Y(k) = std_Y;
    fixed_results_std_Z(k) = std_Z;
end
        end
    end
...
% =========================================================================
figure('Name', 'Академическое SVD-решение устойчивой ковариации');
...
title(sprintf('Облако оценок GN (Станций = %d)', FIXED_N_STATIONS));

% [В блоке отрисовки Сабплота 2]
subplot(2,2,2);
plot(x_true/1000, fixed_results_std_X, 'g-', 'LineWidth', 1.5); hold on;
plot(x_true/1000, fixed_results_std_Y, 'b-', 'LineWidth', 1.5);
plot(x_true/1000, fixed_results_std_Z, 'r-', 'LineWidth', 1.5);
grid on; xlabel('X, km'); ylabel('Calculated STD, meters');
xlim([-20 20]);
legend('Боковой уход (X)', 'Дальность прилета (Y)', 'Ошибка высоты (Z)', 'Location', 'northeast');
title('Раздельные профили функций промаха');
...
title('Сходимость Bias Y к абсолютному нулю');

fprintf('\n=== ТАБЛИЦА СХОДИМОСТИ: ВЫЛЕТЫ В NaN УСТРАНЕНЫ ===\n');
disp(table(STATION_COUNTS_VECTOR', gn_summary_rmse, gn_summary_bias_y, 'VariableNames', {'Станций', 'RMSE_м', 'Bias_Y_м'}));
