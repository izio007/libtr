function [fixed, summary] = test_system_verificationSVD(STATION_COUNTS_VECTOR, FIXED_N_STATIONS, STATION_X_ANCHORS, STATION_Y_ANCHORS, STATION_Z_ANCHORS, x_true, y_true, z_true, DOA_ERROR_DEGREE, RANDOM_SEED)
% =========================================================================
% ВЕРИФИКАТОР МЕТОДА: SVD-ПОЛЯРНЫЙ ИНВАРИАНТ (ЧИСТЫЙ СБОРЩИК)
% Path: d:\workspace\libtr\matlabtests\test_system_verificationSVD.m
% =========================================================================
...
for s_idx = 1:L_counts
    ...
    status_gn = zeros(1, TRAJECTORY_POINTS_COUNT);
    ...
    for k = 1:TRAJECTORY_POINTS_COUNT
        ...
        for i = 1:N_STATIONS
            dx = x_true(k) - STATION_POSITIONS(1, i); dy = y_true(k) - STATION_POSITIONS(2, i); dz = z_true(k) - STATION_POSITIONS(3, i);
            r_xy = sqrt(dx^2 + dy^2);
            alpha_m(i) = atan2(dy, dx) + deg2rad(DOA_ERROR_DEGREE) * randn();
            beta_m(i)  = atan2(dz, r_xy) + deg2rad(DOA_ERROR_DEGREE) * randn();
        end
        
        % 1. Расчет координат через атомарное ядро позиционирования GN
        [st_p, lambda] = gn3d_position(STATION_POSITIONS, alpha_m, beta_m);
        status_gn(k) = st_p;
        
        if st_p == 0
            results_pos(:, k) = lambda;
            % 2. Вызов НОВОГО изолированного ядра инвариантной ковариации
            [st_c, sx, sy, sz] = gn3d_covariance_invariant(STATION_POSITIONS, VAR_ALPHA, VAR_BETA, lambda);
            if st_c == 0
                results_std_X(k) = sx; results_std_Y(k) = sy; results_std_Z(k) = sz;
            end
        end
    end
    ...
    mask = (status_gn == 0);
    if any(mask)
        err_x = results_pos(1, mask) - x_true(mask); err_y = results_pos(2, mask) - y_true(mask); err_z = results_pos(3, mask) - z_true(mask);
    ...
    if N_STATIONS == FIXED_N_STATIONS
        fixed.pos = results_pos; fixed.std_X = results_std_X; fixed.std_Y = results_std_Y; fixed.std_Z = results_std_Z;
