...
FIXED_N_STATIONS = STATION_COUNTS_VECTOR(14); 
%%%%%%%%%%%%%%%

DOA_ERROR_DEGREE = 2.0; % Проводим честный стресс-тест при шуме 2 градуса!
RANDOM_SEED = 1337;


t_steps = linspace(0, 1, TRAJECTORY_POINTS_COUNT);
x_true = TARGET_X_START + (TARGET_X_FINISH - TARGET_X_START) * t_steps;
y_true = TARGET_Y_STATIC * ones(size(t_steps)); z_true = TARGET_Z_STATIC * ones(size(t_steps));

% Расчет общего независимого декартова репера CRLB Фишера со знанием дальности
max_N = max(STATION_COUNTS_VECTOR);
t_q_max = linspace(1, length(STATION_X_ANCHORS), max_N);
P_max = [spline(1:4, STATION_X_ANCHORS, t_q_max); spline(1:4, STATION_Y_ANCHORS, t_q_max); spline(1:4, STATION_Z_ANCHORS, t_q_max)];
crlb.X = zeros(TRAJECTORY_POINTS_COUNT, 1); crlb.Y = zeros(TRAJECTORY_POINTS_COUNT, 1); crlb.Z = zeros(TRAJECTORY_POINTS_COUNT, 1);
...
matplot_verification_screen(x_true, y_true, STATION_X_ANCHORS, STATION_Y_ANCHORS, f_lls, s_lls, crlb, STATION_COUNTS_VECTOR, FIXED_N_STATIONS, '1. Чистый линейный LLS');
...
matplot_verification_screen(x_true, y_true, STATION_X_ANCHORS, STATION_Y_ANCHORS, f_wlls, s_wlls, crlb, STATION_COUNTS_VECTOR, FIXED_N_STATIONS, '2. Взвешенный WLLS (Гаусс-веса)');
...
matplot_verification_screen(x_true, y_true, STATION_X_ANCHORS, STATION_Y_ANCHORS, f_gn, s_gn, crlb, STATION_COUNTS_VECTOR, FIXED_N_STATIONS, '3. Декартов Gauss-Newton');
...
matplot_verification_screen(x_true, y_true, STATION_X_ANCHORS, STATION_Y_ANCHORS, f_pol, s_pol, crlb, STATION_COUNTS_VECTOR, FIXED_N_STATIONS, '4. Истинный полярный инвариант');
