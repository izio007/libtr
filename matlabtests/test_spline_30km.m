...
for s_idx = 1:length(STATION_COUNTS_VECTOR)
    ...
    for k = 1:TRAJECTORY_POINTS_COUNT
        ...
        if st_p == 0
            ...
            [st_c, ~, std_3d] = lls3d_covariance(STATION_POSITIONS, alpha_measured, beta_measured, VAR_ALPHA, VAR_BETA, lambda);
            if st_c == 0
    ...
    if any(valid_mask)
        ...
        % Если из-за вырождения МНК ни один такт не посчитался
...
% =========================================================================
% ОКНО 1: СТАРЫЙ ДЕТАЛЬНЫЙ ФОРМАТ (ДЛЯ СЛУЧАЯ 4 СТАНЦИЙ)
% =========================================================================
figure('Name', sprintf('Детальный анализ траектории для %d станций', FIXED_N_STATIONS));
subplot(2,2,1);
...
% =========================================================================
% ОКНО 2: СВОДНЫЕ ГРАФИКИ СХОДИМОСТИ
% =========================================================================
subplot(2,2,3);
