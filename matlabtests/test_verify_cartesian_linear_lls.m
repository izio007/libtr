...
for s_idx = 1:L_counts
    ...
    for k = 1:TRAJECTORY_POINTS_COUNT
        ...
        [st_p, lambda] = lls3d_position(STATION_POSITIONS, alpha_m, beta_m);
        status(k) = st_p;
        if st_p == 0
            results_pos(:, k) = lambda;
            [st_c, sx, sy, sz] = lls3d_covariance_linear(STATION_POSITIONS, alpha_m, beta_m, VAR_ALPHA, VAR_BETA, lambda);
            if st_c == 0
